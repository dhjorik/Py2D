...
class Viewport:
    ...
    def update(self):
        ...
        if self.game.player.moving and (not self.game.player.mirrored):
        ...
        if self.game.player.moving and self.game.player.mirrored:
            if self.player_x > 0:
                self.player_x -= self.player_speed
                self.scroll_x -= self.player_speed
                direction = -1
        ...
        self.level.player_y = self.player_y + round(self.player.actor_jump_y)
        self.level.player_sprite = self.player.actor_layer
        self.level.update()
